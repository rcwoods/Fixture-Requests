...
| `fixture_requests_vW26.py` | The scheduling engine. Now guarded so importing it no longer triggers a run. |
| `test_scheduler.py` | Safety-net checks: valid-schedule invariants plus an optional regression test. |
...
```
python test_scheduler.py                 # run the engine, then check the result
python test_scheduler.py --no-run        # just check the current output
python test_scheduler.py --update-baseline   # save the current schedule as the regression baseline
```

It always checks the hard invariants (no court double-booked, no team in two
games at once). If you save a baseline first, it also tells you exactly how many
rows changed after an edit, so a refactor that is meant to change nothing can be
proven to change nothing. It also reports the soft trade-offs (unavailability
windows not honoured) so you can watch them as you tune.

The engine is now import-safe: it only schedules and writes files when run
directly (or via the app / `run_fixtures.py`), so the test can import it without
side effects.
